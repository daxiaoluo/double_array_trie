...
struct BaseItem {
	...
	BaseItem(list<char> &tail_list): val(0), tail(tail_list.end()) {}
};
...
private:
	...
	bool resolvePrefixConflict(int index, const string& str, int str_index);
	bool resolveBaseConflict(int base_index, int check_index);
	int getNewBase(int cur_index, char s);
	int getNewBase(int cur_index, char s1, char s2, int &prefix1_index, int &prefix2_index);
public:
